...
class ImageCollectionBodyPartService {
  ...
  List<ListCollectionBodyPart> parseImageInCollectionList(String responseBody, int index) {
    int count = 0;
    ...
    for (int i = 0; i < count; i++) {
      imageList.add(ListCollectionBodyPart.fromJson(list['listCollectionProject'][i]['imageList']));
    }
    return imageList;
  }

  Future<List<ListCollectionBodyPart>> getImageInCollectionList(int index) async {
    final response = await http
        .get(Uri.parse(url + "api/v1/models/28"));
